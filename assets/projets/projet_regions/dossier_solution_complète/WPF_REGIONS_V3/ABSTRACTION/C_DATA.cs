...
namespace LIB_REGION
{
  ...
  internal class C_DATA
  {
    ...
    public List<C_DEPARTEMENT> Get_Departements_By_Region(C_REGION P_Region)
    {
      ...
      foreach (var Un_Departement in La_Base.Les_Depatements) {
    ...
    public List<C_VILLE> Get_Villes_By_Departement(C_DEPARTEMENT P_Departement)
    {
      ...
      foreach (var Une_Ville in La_Base.Les_Villes) {
    ...
    public List<C_EMETTEUR> Get_All_Emetteurs_By_Code_Postal(string P_Code)
    {
      ...
      for (int Index = 0; Index < La_Base.Les_Emetteurs.Count; Index++) {
        if (La_Base.Les_Emetteurs[Index].CP == P_Code) {
          Emetteurs_Trouves.Add(La_Base.Les_Emetteurs[Index]);
        }
      }
